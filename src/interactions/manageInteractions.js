...
async function handleManageInteraction(interaction, client) {
  ...
  if (id.startsWith('mgr_gen_matches_')) {
    ...
    for (const groupTeams of Object.values(groups)) {
      // Circle round-robin: each pair plays exactly once per round
      const arr = [...groupTeams];
      if (arr.length % 2 !== 0) arr.push(null); // bye slot for odd counts
        ...
        for (let i = 0; i < half; i++) {
          ...
          if (home && away) {
            allMatches.push(db.insert('matches', {
              ...
              stage: 'group', round: r + 1, leg: 1, status: 'pending',
