...
async function doPublish(interaction, buildFn, preferredChKey = 'results') {
  ...
  const msg     = buildFn();
  if (!msg) return interaction.reply({ content: '\u274C Could not build post.', ephemeral: true });
  if (preview) return interaction.reply({ ...msg, ephemeral: true });

  const targetChId = ch[preferredChKey] || ch.management;
  if (!targetChId) return interaction.reply({ content: '\u274C No channel set. Set a Results or Schedule channel in Channels & Roles.', ephemeral: true });
  ...
  await interaction.reply({ flags: 64 | 32768, components: [{ type: 17, accent_color: 0x57F287, components: [
...
async function doPublishLive(interaction, buildFn, preferredChKey, liveRefKey) {
  ...
  const targetChId = ch[preferredChKey] || ch.management;
  if (!targetChId) return interaction.reply({ content: '\u274C No channel set.', ephemeral: true });
  const target = interaction.guild.channels.cache.get(targetChId) || await interaction.guild.channels.fetch(targetChId).catch(() => null);
  if (!target) return interaction.reply({ content: '\u274C Channel not found.', ephemeral: true });
...
async function handleFederationInteraction(interaction, client) {
  ...
  if (id === 'fed_p3_standings') {
    return doPublishLive(interaction, () => {
      ...
      const mts     = getFedMatches();
      const isLeague= (fed.system || 'cup') === 'league';
      return makeFedStandingsPost(fed, mts, clans, isLeague);
  ...
  if (id === 'fed_p3_bracket') {
    ...
    if (!msg) return interaction.reply({ content: '\u274C Could not build bracket.', ephemeral: true });
    const preview = fed.p3_preview === true;
    if (preview) return interaction.reply({ ...msg, ephemeral: true });
    const ch      = fed.channels || {};
    const targetChId = ch.results || ch.management;
    if (!targetChId) return interaction.reply({ content: '\u274C No results channel set.', ephemeral: true });
    ...
    await interaction.reply({ flags: 64 | 32768, components: [{ type: 17, accent_color: 0x57F287, components: [
      { type: 10, content: '\u2705 Bracket posted to <#' + targetChId + '> \u2014 updates live as results are added.' },
    ]}]});
    setTimeout(() => interaction.deleteReply().catch(() => {}), 4000);
    return;
  }
