...
// ── Channels config panel (inline, not ephemeral) ────────────────────────────
function buildChannelsPanel() {
  ...
  return {
    flags: 32768,
    components: [{ type: 17, accent_color: BLUE, components: [
      txt('📺  **Set Test Channels**\n*Select a channel for each category. Changes save instantly.*'),
      SEP,
      txt(`Results & Standings  →  ${rId ? `<#${rId}>` : '`not set`'}`),
      mkPicker('Results & Standings', 'results', rId),
      SEP,
      txt(`Schedule  →  ${sId ? `<#${sId}>` : '`not set`'}`),
      mkPicker('Schedule', 'schedule', sId),
      SEP,
      { type: 1, components: [{ type: 2, style: 2, label: '← Back', custom_id: 'at_ch_back' }] },
      SEP,
  ...
  return {
    ...
    components: [{
      ...
      components: [
        ...
        { type: 1, components: [{ type: 2, style: 2, label: '📺  Set Test Channels', custom_id: 'at_set_channels' }] },
        SEP,
        txt('-# © 24 2026  |  Goatsi Bot'),
      ],
    }],
    ...
    for (let ko = 0; ko < numKoRounds; ko++) {
      ...
      steps.push(async () => {
        ...
        if (isLast) {
          ...
          if (results) await results.send(makeChampionPost('Test', 1, champTeam.name));
        }
      });
    }
  } else {
...
async function handleAutotestInteraction(interaction, client) {
  ...
  if (id === 'at_set_channels') {
    return interaction.update(buildChannelsPanel());
  }
  ...
  if (id === 'at_ch_results' || id === 'at_ch_schedule') {
    const cfgKey = id === 'at_ch_results' ? 'test_results_channel_id' : 'test_schedule_channel_id';
    const val    = (interaction.values && interaction.values[0]) || null;
    db.setConfig(cfgKey, val);
    return interaction.update(buildChannelsPanel());
  }
