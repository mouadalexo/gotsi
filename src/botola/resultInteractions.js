...
async function handleResultInteraction(interaction, client) {
  ...
  // ── Select tournament to view bracket ─────────────────────────────────────
  if (id === 'tournament_bracket') {
    const menu = buildTournamentSelectMenu('Select a tournament...', 'bracket_tournament_select', { showFinished: true });
    if (!menu) return interaction.reply({ embeds: [warningEmbed('No Tournaments', 'No tournaments found.')], ephemeral: true });
    return interaction.reply({ content: '📋  Select a tournament to view its bracket:', components: [menu], ephemeral: true });
  }
  ...
  if (id.startsWith('result_modal_')) {
    ...
    // Update group standings using tournament-configured points
