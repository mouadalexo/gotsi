...
function buildEnrollStep2(tid, opts = {}) {
  ...
  const isFull = !!(tournament.team_count && enrolledIds.length >= tournament.team_count);

  const inner = [];
  if (isFull) {
    inner.push(txt('## \u2795  Enroll Team\n> **' + tournament.name + '** is full \u2014 **' + enrolledIds.length + '/' + tournament.team_count + '** teams registered.\n> Registration is now closed for this tournament.'));
    if (error) inner.push(txt('> \u26a0\ufe0f  ' + error));
    inner.push(SEP);
    inner.push({ type: 1, components: [{ type: 2, style: 2, label: '\u2190 Back', custom_id: 'enr_back_step1' }] });
