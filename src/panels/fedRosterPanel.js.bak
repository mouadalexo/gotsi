...
function buildRosterLauncher(member) {
  ...
  return { components: [{ type: 17, accent_color: 0x9B59B6, components: inner }] };
}
...
function buildLeaderDashboard(leaderId, opts = {}) {
  ...
  inner.push({ type: 1, components: [
    ...
    btn('📄  Download PDF', 'fr_pdf',     2, !canPdf),
  ]});
  inner.push({ type: 1, components: [
  ...
  return { flags: 64, components: [{ type: 17, accent_color: 0x9B59B6, components: inner }] };
}
...
function buildPickUserPanel(slot, opts = {}) {
  ...
  const inner = [
    txt(
      '## ➕  Add Player — Step 1 of 2\n' +
      '> Select a **server member** for slot **#' + slot + '**.\n' +
      '> Members already registered in this federation are excluded.'
    ),
  ];
  if (error) inner.push(txt('> ❌  ' + error));
  inner.push(SEP);
  ...
  return { flags: 64, components: [{ type: 17, accent_color: 0x5865F2, components: inner }] };
}
...
function buildEditPlayerSelect(leaderId) {
  ...
  const inner = [
    txt('**✏️  Edit Player** — select a player:'),
    SEP,
    { type: 1, components: [{
      type: 3,
...
function buildRemovePlayerSelect(leaderId) {
  ...
  const inner = [
    txt('**❌  Remove Player** — select a player:'),
    SEP,
    { type: 1, components: [{
      type: 3,
...
function buildConfirmRemove(leaderId, slot) {
  ...
  return { flags: 64, components: [{ type: 17, accent_color: 0xFF0000, components: inner }] };
}
...
function buildAdminClanView(rosterId, opts = {}) {
  ...
  inner.push({ type: 1, components: [
    ...
    btn('📄  Download PDF', 'fra_pdf_' + rosterId, 2),
  ]});
  inner.push({ type: 1, components: [
  ...
  return { flags: 64, components: [{ type: 17, accent_color: accentColor, components: inner }] };
}
...
function buildAdminSettings(opts = {}) {
  ...
  return { flags: 64, components: [{ type: 17, accent_color: 0xFEE75C, components: inner }] };
}
